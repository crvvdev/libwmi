...
std::string Wmi::wstrtostr(const std::wstring& wstr) {
  int sizeNeeded =
      WideCharToMultiByte(CP_ACP, 0, wstr.c_str(),
                          static_cast<int>(wstr.size()), NULL, 0, NULL, NULL);
  if (sizeNeeded) {
    std::string str(sizeNeeded, 0);
    if (WideCharToMultiByte(CP_ACP, 0, wstr.c_str(),
                            static_cast<int>(wstr.size()), &str[0], sizeNeeded,
                            NULL, NULL)) {
      return str;
    }
  }
...
std::wstring escape(std::wstring str) {
  size_t start_pos = 0;
  ...
  if (FAILED(hr)) {
    switch (hr) {
      case REGDB_E_CLASSNOTREG:
        throw WmiException(
            "Error initializing IWbemLocator: REGDB_E_CLASSNOTREG", hr);
      case CLASS_E_NOAGGREGATION:
        throw WmiException(
            "Error initializing IWbemLocator: CLASS_E_NOAGGREGATION", hr);
      case E_NOINTERFACE:
      ...
      default:
        throw WmiException("Error initializing IWbemLocator: Unknown Error",
                           hr);
    }
  }
...
IWbemServices* connect(IWbemLocator* pLocator, const std::string& path) {
  ...
  HRESULT hr = pLocator->ConnectServer(
      _bstr_t("\\\\.\\root\\") + _bstr_t(path.c_str()), nullptr, nullptr,
      nullptr, 0, nullptr, nullptr, &pServices);
  if (FAILED(hr)) {
    switch (hr) {
      case (HRESULT)WBEM_E_ACCESS_DENIED:
        throw WmiException(
            "Error initializing IWbemServices: WBEM_E_ACCESS_DENIED", hr);
      case (HRESULT)WBEM_E_FAILED:
        throw WmiException("Error initializing IWbemServices: WBEM_E_FAILED",
                           hr);
      case (HRESULT)WBEM_E_INVALID_NAMESPACE:
        throw WmiException(
            "Error initializing IWbemServices: WBEM_E_INVALID_NAMESPACE", hr);
      case (HRESULT)WBEM_E_INVALID_PARAMETER:
        throw WmiException(
            "Error initializing IWbemServices: WBEM_E_INVALID_PARAMETER", hr);
      case (HRESULT)WBEM_E_OUT_OF_MEMORY:
        throw WmiException(
            "Error initializing IWbemServices: WBEM_E_OUT_OF_MEMORY", hr);
      case (HRESULT)WBEM_E_TRANSPORT_FAILURE:
        throw WmiException(
            "Error initializing IWbemServices: WBEM_E_TRANSPORT_FAILURE", hr);
      case (HRESULT)WBEM_E_LOCAL_CREDENTIALS:
        throw WmiException(
            "Error initializing IWbemServices: WBEM_E_LOCAL_CREDENTIALS", hr);
      default:
        throw WmiException("Error initializing IWbemServices: Unknown Error",
                           hr);
    }
  }

  // Set authentication proxy
  hr = CoSetProxyBlanket(pServices, RPC_C_AUTHN_DEFAULT, RPC_C_AUTHZ_NONE,
  ...
  if (FAILED(hr)) {
    switch (hr) {
      case E_INVALIDARG:
        throw WmiException("Coult not set proxy blanket: E_INVALIDARG", hr);
      default:
        throw WmiException("Coult not set proxy blanket: Unknown Error", hr);
    }
  }
...
IEnumWbemClassObject* execute(IWbemServices* pServices, const std::string& q) {
  ...
  HRESULT hr = pServices->ExecQuery(
      _bstr_t("WQL"), _bstr_t(q.c_str()),
      WBEM_FLAG_RETURN_IMMEDIATELY | WBEM_FLAG_FORWARD_ONLY, nullptr,
      &pClassObject);
  if (FAILED(hr)) {
    switch (hr) {
      case (HRESULT)WBEM_E_ACCESS_DENIED:
      ...
      case (HRESULT)WBEM_E_INVALID_PARAMETER:
        throw WmiException("Error executing query: WBEM_E_INVALID_PARAMETER",
                           hr);
      case (HRESULT)WBEM_E_OUT_OF_MEMORY:
      ...
      case (HRESULT)WBEM_E_TRANSPORT_FAILURE:
        throw WmiException("Error executing query: WBEM_E_TRANSPORT_FAILURE",
                           hr);
      default:
...
void foreachObject(IEnumWbemClassObject* pClassObject,
                   std::function<bool(IWbemClassObject*)> fn) {
  bool cont = true;
  ...
  while (cont && hr == WBEM_S_NO_ERROR) {
    ...
    if (FAILED(hr)) {
      switch (hr) {
        ...
        case (HRESULT)WBEM_E_INVALID_PARAMETER:
          throw WmiException(
              "Error getting next element: WBEM_E_INVALID_PARAMETER", hr);
        case (HRESULT)WBEM_E_OUT_OF_MEMORY:
          throw WmiException("Error getting next element: WBEM_E_OUT_OF_MEMORY",
                             hr);
        case (HRESULT)WBEM_E_UNEXPECTED:
          throw WmiException("Error getting next element: WBEM_E_UNEXPECTED",
                             hr);
        case (HRESULT)WBEM_E_TRANSPORT_FAILURE:
          throw WmiException(
              "Error getting next element: WBEM_E_TRANSPORT_FAILURE", hr);
...
std::wstring convertVariant(const VARIANT& value) {
  ...
  switch (value.vt) {
    ...
    case VT_USERDEFINED:
      throw WmiException("Data type not yet supported: VT_USERDEFINED",
                         value.vt);
    case VT_RECORD:
...
void foreachProperty(
    ...
    std::function<bool(const std::wstring&, const std::wstring&)> fn) {
  SAFEARRAY* psaNames = nullptr;
  HRESULT hr = object->GetNames(
      nullptr, WBEM_FLAG_ALWAYS | WBEM_FLAG_NONSYSTEM_ONLY, nullptr, &psaNames);
  ...
  if (FAILED(hr)) {
    switch (hr) {
      ...
      case (HRESULT)WBEM_E_INVALID_PARAMETER:
        throw WmiException("Could not get properties: WBEM_E_INVALID_PARAMETER",
                           hr);
      case (HRESULT)WBEM_E_OUT_OF_MEMORY:
        throw WmiException("Could not get properties: WBEM_E_OUT_OF_MEMORY",
                           hr);
      default:
        throw WmiException("Could not get properties: WBEM_E_FAILED", hr);
    }
  ...
  for (long i = lLower; i <= lUpper; ++i) {
    ...
    if (FAILED(hr)) {
      switch (hr) {
        case DISP_E_BADINDEX:
          throw WmiException(
              "Could not get name from SafeArray: DISP_E_BADINDEX", hr);
        case E_INVALIDARG:
          throw WmiException("Could not get name from SafeArray: E_INVALIDARG",
                             hr);
        case E_OUTOFMEMORY:
          throw WmiException("Could not get name from SafeArray: E_OUTOFMEMORY",
                             hr);
        default:
          throw WmiException("Could not get name from SafeArray: Unknown Error",
                             hr);
      }
    }
    ...
    if (FAILED(hr)) {
      switch (hr) {
        ...
        case (HRESULT)WBEM_E_INVALID_PARAMETER:
          throw WmiException("Could not get property: WBEM_E_INVALID_PARAMETER",
                             hr);
        case (HRESULT)WBEM_E_NOT_FOUND:
          throw WmiException("Could not get property: WBEM_E_NOT_FOUND", hr);
        case (HRESULT)WBEM_E_OUT_OF_MEMORY:
          throw WmiException("Could not get property: WBEM_E_OUT_OF_MEMORY",
                             hr);
        default:
    ...
    try {
      ...
      throw WmiException(std::string("Can't convert parameter: ") +
                             Wmi::wstrtostr(tmp) + ": " + e.errorMessage,
                         e.errorCode);
    }
...
void Wmi::query(const std::string& q, const std::string& p, WmiResult& out) {
  HRESULT hr = CoInitialize(nullptr);
  if (FAILED(hr)) {
    throw WmiException("The COM library is already initialized on this thread",
                       hr);
  }
  ...
  try {
    ...
    foreachObject(pClassObject, [&out, &index](IWbemClassObject* object) {
      foreachProperty(object, [&out, index](const std::wstring& name,

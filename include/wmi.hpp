...
template <class WmiClass>
typename std::enable_if<std::is_function<decltype(WmiClass::getWmiPath)>::value,
                        std::string>::type
...
inline void retrieveWmi(WmiClass& out) {
  WmiResult result;
  const std::string q =
      std::string("SELECT * FROM ") + WmiClass::getWmiClassName();
...
inline void retrieveWmi(WmiClass& out, std::string columns) {
  WmiResult result;
  const std::string q = std::string("SELECT ") + columns +
                        std::string(" FROM ") + WmiClass::getWmiClassName();
  query(q, CallGetWmiPath<WmiClass>(0), result);
  out.setProperties(result, 0);
}
